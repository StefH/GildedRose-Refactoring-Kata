        ...
        for (var i = 0; i < _items.Count; i++)
        {
            ...
            else
            {
                if (_items[i].Quality < MAX_ITEM_QUALITY)
                {
                    ...
                    {
                        if (_items[i].SellInDays < CONCERT_NEAR_DAYS)
                        {
                            if (_items[i].Quality < MAX_ITEM_QUALITY)
                            {
                                _items[i].Quality += DAILY_QUALITY_INCREASE;
                        ...
                        if (_items[i].SellInDays < CONCERT_VERY_NEAR_DAYS)
                        {
                            if (_items[i].Quality < MAX_ITEM_QUALITY)
                            {
                                _items[i].Quality += DAILY_QUALITY_INCREASE;
            ...
            if (_items[i].Name != ITEM_SULFURAS)
            {
                _items[i].SellInDays -= DAILY_QUALITY_DECREASE;
            }

            if (_items[i].SellInDays < MIN_SELLIN)
            {
